...
void drawbar(Monitor *m) {
    ...
    for (i = 0; i < LENGTH(tags); i++) {
        ...
        drw_setscheme(
            drw,
            scheme[m->tagset[m->seltags] & 1 << i ? SchemeAct : SchemeNorm]);
    ...
    if ((w = m->ww - tw - x) > bh) {
        if (m->sel) {
            ...
            int tlpad = MAX((m->ww - ((int)TEXTW(m->sel->name) - lrpad)) / 2 - x, lrpad / 2);
			drw_text(drw, x, 0, w, bh, tlpad, m->sel->name, 0);
            if (m->sel->isfloating)
                drw_rect(
                    drw, x + boxs + tlpad - lrpad / 2, 
                    boxs, boxw, boxw, m->sel->isfixed, 0
                );
        } else {
            drw_setscheme(drw, scheme[SchemeNorm]);
